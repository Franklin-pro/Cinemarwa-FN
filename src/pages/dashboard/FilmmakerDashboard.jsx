...
// const handleManageSeries = (seriesId) => {
//   navigate(`/filmmaker/series/${seriesId}/edit`);
// };
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      ...
                      >
                        Account ID:{" "}
                        {dashboardData?.user?._id?.slice(-6) || "XXXXXX"}
                      </p>
                    </div>
    ...
    <div className="flex justify-between items-center mb-6">
      <div>
        <h2 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>
          My Content ({movies.length})
        </h2>
        <div className="flex gap-2 mt-2">
          <span className={`text-sm px-2 py-1 rounded ${darkMode ? "bg-blue-900/50 text-blue-300" : "bg-blue-100 text-blue-700"}`}>
            {movies.filter(m => m.contentType === "movie").length} Movies
          </span>
          <span className={`text-sm px-2 py-1 rounded ${darkMode ? "bg-purple-900/50 text-purple-300" : "bg-purple-100 text-purple-700"}`}>
    ...
    {movies.length === 0 ? (
      <div className={`${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} border rounded-xl p-12 text-center transition-colors duration-200`}>
        ...
        <div className="flex gap-4 justify-center">
          ...
          <button
            onClick={() => navigate("/dashboard/filmmaker/create-series")}
        ...
        {movies.map((content) => {
          ...
          return (
            <div
              ...
              } rounded-lg overflow-hidden border transition-all group cursor-pointer`}
              onClick={() => {
                if (isSeries) {
            ...
            >
              ...
              <div className="p-4 space-y-2" onClick={(e) => e.stopPropagation()}>
                ...
                <div className={`text-xs space-y-1 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
                  {isMovie && (
                    <>
                      <p>Duration: {formatDuration(content.videoDuration)}</p>
                      <p>Views: {formatNumber(content.totalViews || 0)}</p>
                      <p>Revenue: ${formatCurrency(content.totalRevenue || 0)}</p>
                    </>
                  )}
                  
                  {isSeries && (
                    <>
                      <p>Seasons: {content.totalSeasons || 1}</p>
                      <p>Episodes: {content.totalEpisodes || content.episodes?.length || 0}</p>
                      <p>Total Views: {formatNumber(content.totalViews || 0)}</p>
                    </>
                  )}
                ...
                {isSeries && content.episodes && content.episodes.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div className="space-y-1">
                      {content.episodes.slice(0, 3).map((episode) => (
                        <div 
                          key={episode.id} 
                          className="flex items-center justify-between text-xs hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded cursor-pointer"
                          onClick={(e) => {
                            e.stopPropagation();
                      ...
                      {content.episodes.length > 3 && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleViewSeriesEpisodes(content.id);
                          }}
                          className="text-xs text-blue-600 dark:text-blue-400 hover:underline text-center w-full py-1"
                        >
                          +{content.episodes.length - 3} more episodes
                        </button>
                      )}
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  >
                    ...
                    >
                      ⚠️ Minimum withdrawal: $50 | Processing time: 5-7 business
                      days
                    </p>
                  </div>
                  <button
                            ...
                            <span
                              className={`${
                                payment.paymentStatus === "success"
                                  ? "bg-green-500/15 rounded-full text-green-500"
                                  : payment.paymentStatus === "pending"
                                  ? "bg-yellow-500/15 rounded-full text-yellow-500"
                                  : "bg-red-500/15 px-4 py-1 rounded-full text-red-500"
